...
if [ "$VERIFY_ONLY" = "0" ]; then
  ...
  # O volume pode ter sido criado por uma instalação anterior sem as roles
  # do Supabase. Elas são criadas separadamente para que uma falha fique
  # explícita e não deixe o bootstrap continuar em estado parcial.
  psql_root -v ON_ERROR_STOP=1 -c "DO \\$\$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='anon') THEN CREATE ROLE anon NOLOGIN NOINHERIT; END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='authenticated') THEN CREATE ROLE authenticated NOLOGIN NOINHERIT; END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='service_role') THEN CREATE ROLE service_role NOLOGIN NOINHERIT BYPASSRLS; END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='authenticator') THEN CREATE ROLE authenticator LOGIN NOINHERIT; END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='supabase_admin') THEN CREATE ROLE supabase_admin LOGIN NOINHERIT CREATEROLE CREATEDB BYPASSRLS; END IF;
  ...
  if dc exec -T funecob-db test -f /opt/funecob/bootstrap.sql 2>/dev/null; then
    ...
    dc exec -T funecob-db psql -v ON_ERROR_STOP=1 \
      -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-postgres}" \
      -v db_password="${POSTGRES_PASSWORD}" -q < "$BOOTSTRAP_LOCAL" \
      || die "Bootstrap do PostgreSQL falhou — migrations NÃO foram executadas"
  fi
...
for r in anon authenticated service_role authenticator supabase_admin supabase_auth_admin supabase_storage_admin; do
