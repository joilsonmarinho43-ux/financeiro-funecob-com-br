      ...
      for (const invoice of invoices) {
        ...
        try {
          ...
          if (existingToken?.token) {
            portalLink = `${Deno.env.get("SUPABASE_URL")?.replace(".supabase.co", ".lovable.app") || ""}/portal/${existingToken.token}`;
          } else {
            const { data: newToken } = await supabase
              ...
              .single();
            if (newToken?.token) {
              portalLink = `${Deno.env.get("SUPABASE_URL")?.replace(".supabase.co", ".lovable.app") || ""}/portal/${newToken.token}`;
            }
          }
        } catch (e) {
