...
// Returns only controlled Brazilian mobile variants (10/11 digits), never partial suffixes.
...
function strongNameTokens(s: string): string[] {
  return normName(s).split(/\s+/).map((t) => t.trim()).filter((t) => t.length >= 3 && !NAME_STOPWORDS.has(t));
}
...
function tokenMatchesName(sourceToken: string, clientTokens: Set<string>): boolean {
  ...
  for (const clientToken of clientTokens) if (clientToken.length >= 5 && editDistanceWithinOne(sourceToken, clientToken)) return true;
  return false;
}
...
async function matchesOpenInvoicesByAmount(supabase: any, organizationId: string, clientId: string, amount: number | null): Promise<boolean> {
  if (!amount) return false;
  const { data: openInvs } = await supabase.from("invoices").select("amount").eq("organization_id", organizationId).eq("client_id", clientId).eq("status", "aberto");
  const amounts = (openInvs || []).map((i: any) => Number(i.amount));
  return amounts.some((invoiceAmount) => Math.abs(invoiceAmount - Number(amount)) < 0.01) || !!findExactCombination(amounts, Number(amount));
}
...
async function runGeminiDirectOcr(mediaUrl: string, mimeType: string, amountOnly = false): Promise<any> {
  ...
  try {
    ...
    if (listRes.ok) {
      ...
      const available = (listed?.models || []).filter((m: any) => Array.isArray(m?.supportedGenerationMethods) && m.supportedGenerationMethods.includes("generateContent")).map((m: any) => String(m?.name || "").replace(/^models\//, "")).filter((name: string) => /gemini/i.test(name) && !/embedding|tts|imagen/i.test(name));
      const ordered = [...preferredModels.filter((m) => available.includes(m)), ...available.filter((m: string) => !preferredModels.includes(m) && /(flash|pro)/i.test(m))];
      if (ordered.length) models = ordered;
    } else console.warn("[pix-ocr] Gemini listModels failed", listRes.status, (await listRes.text()).slice(0, 200));
  } catch (e) { console.warn("[pix-ocr] Gemini listModels error (using defaults)", String((e as any)?.message || e)); }
  ...
  for (const model of models) {
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(GEMINI_API_KEY)}`, {
      ...
      body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: prompt }, { inline_data: { mime_type: mimeType || "image/jpeg", data: dataUrlToBase64(mediaUrl) } }] }], generationConfig: { response_mime_type: "application/json" } }),
    });
    if (!res.ok) { const msg = `Gemini ${model} failed [${res.status}]: ${(await res.text()).slice(0, 500)}`; failures.push(msg); console.warn("[pix-ocr] Gemini model failed", msg); continue; }
...
async function runOcrAmountOnly(mediaUrl: string, mimeType: string): Promise<{ amount: number | null; raw_text: string | null }> {
  ...
  } catch (e) { console.warn("[pix-ocr] 2nd-pass amount-only failed (non-blocking)", e); return { amount: null, raw_text: null }; }
}

async function processEvent(supabase: any, eventId: string, organizationId: string) {
  try {
    const { data: ev } = await supabase.from("auto_settlement_events").select("*").eq("id", eventId).maybeSingle();
    if (!ev || ["duplicado", "conciliado"].includes(ev.status)) return;
    ...
    if (result?.success && result?.skipped !== "duplicate_recent_paid" && ev.client_id && ev.amount_detected) {
      ...
      const { data: confirmationLog } = await supabase.from("auto_settlement_logs").select("id").eq("event_id", ev.id).eq("action", "confirmation_sent").limit(1).maybeSingle();
      if (!confirmationLog?.id) {
        try { await deliverPaymentConfirmation(supabase, organizationId, ev); } catch (e) { console.warn("[confirmation] delivery failed after successful settlement", e); }
      }
    }
  } catch (e) { console.error("[pix-ocr] processEvent failed", e); throw e; }
}
  ...
  try {
    ...
    if (!client && clients?.length) {
      ...
      const tryFuzzy = (rawName: string) => {
        ...
        for (const c of clients || []) { const score = strongNameScore(rawName, c.name || ""); if (score > bestScore) { bestScore = score; best = c; } }
        return bestScore >= 1 ? best : null;
      };
      if (!client) for (const n of names) { const candidate = tryFuzzy(n); if (candidate) { client = candidate; matchSource = "fuzzy_name"; break; } }
    }

    let amountMatchesInvoice = false;
    let combinationPicks: number[] | null = null;
    if (client && amount) {
      const { data: openInvs } = await supabase.from("invoices").select("id,amount").eq("organization_id", organization_id).eq("client_id", client.id).eq("status", "aberto");
      const amounts = (openInvs || []).map((i: any) => Number(i.amount));
    ...
    if (!client) { eventStatus = "pendente_revisao"; errorMessage = "cliente não identificado automaticamente"; }
    else if (!amountMatchesInvoice) { eventStatus = "pendente_revisao"; errorMessage = "valor pago não corresponde a nenhuma fatura aberta (nem combinação) — revise"; }
    ...
    const scoreResult = computeScore({ client_identified: !!client, amount_found: !!amount, txid_found: !!(ocr?.txid || ocr?.end_to_end_id), name_compatible: nameCompat, payer_known: payerKnown, single_open_match: amountMatchesInvoice && (combinationPicks?.length || 1) === 1, match_source: matchSource });

    const { data: event, error: eventError } = await supabase.from("auto_settlement_events").insert({
      organization_id,
