...
BEGIN
  ...
  /*
   * PROTEÇÃO DEFINITIVA CONTRA DUPLA BAIXA
   *
   * Se este evento já possui qualquer alocação, significa que
   * uma tentativa anterior já alterou/quitou uma fatura.
   * Nunca processar o mesmo evento novamente.
   */
  SELECT COUNT(*)
  ...
  IF v_existing_allocations > 0 THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'reason', 'evento já possui baixa/alocação; retry bloqueado'
      )
    );
  ...
  IF v_event.client_id IS NULL THEN
    ...
    SET
      status = 'erro',
      error_message = 'client not identified',
      updated_at = now()
    WHERE id = p_event_id;
  ...
  IF v_event.amount_detected IS NULL
     OR v_event.amount_detected <= 0 THEN
  ...
  -- Serializa baixas do mesmo cliente para impedir dois comprovantes
  -- simultâneos de atravessarem a checagem de duplicidade.
  PERFORM pg_advisory_xact_lock(hashtextextended(
    v_event.organization_id::text || ':' || v_event.client_id::text, 0
  ));
  ...
  IF v_recent_dup > 0 THEN
    ...
    VALUES (v_event.organization_id, p_event_id, v_event.client_id,
  ...
  IF v_remaining > 0 THEN
    ...
    IF v_orig_day IS NOT NULL
       AND v_amount IS NOT NULL
       AND v_amount > 0
       AND v_last_due IS NOT NULL THEN

      WHILE v_remaining >= v_amount
            AND v_max_iter > 0 LOOP

        v_max_iter := v_max_iter - 1;

        v_next_year :=
          EXTRACT(YEAR FROM v_last_due)::int;

        v_next_month :=
          EXTRACT(MONTH FROM v_last_due)::int + 1;
        ...
        v_last_day :=
          EXTRACT(
            DAY FROM (
              make_date(v_next_year, v_next_month, 1)
  ...
  IF v_remaining > 0 THEN
    ...
    INSERT INTO public.auto_settlement_credits(
      organization_id,
      client_id,
      amount,
      source,
